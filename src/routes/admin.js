...
export default async function adminRoutes(app) {
  ...
  // Removes the old NYC seed restaurants so only Miami shows on the map.
  app.get('/clear-non-miami', async (req, reply) => {
    ...
    try {
      ...
      const result = await db.query(
        `DELETE FROM restaurants WHERE slug <> ALL($1::text[]) RETURNING name`,
        [miamiSlugs]
      )
      return { success: true, removed: result.rows.map(r => r.name) }
    } catch (err) { return reply.code(500).send({ error: err.message }) }
  })
