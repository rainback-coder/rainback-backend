...
export async function checkinRoutes(app) {
  ...
  // GET /api/checkin/recent/:restaurantId — owner: who just arrived (live feed)
  app.get('/recent/:restaurantId', { preHandler: requireOwner }, async (req, reply) => {
    const r = await db.findOne(
      'SELECT id FROM restaurants WHERE id = $1 AND owner_id = $2',
      [req.params.restaurantId, req.user.id]
    )
    ...
    const checkins = await db.findMany(
      `SELECT ci.id, ci.checked_in_at, ci.party_size,
              u.first_name, u.last_name, m.serial_number, m.tier,
              COALESCE(json_agg(json_build_object('name', mm.dish_name, 'calories', mm.calories))
                FILTER (WHERE mm.id IS NOT NULL), '[]') AS meals
       FROM check_ins ci
       JOIN users u ON u.id = ci.member_id
       JOIN memberships m ON m.id = ci.membership_id
       LEFT JOIN member_meals mm ON mm.member_id = ci.member_id
            AND mm.restaurant_id = ci.restaurant_id
            AND mm.eaten_at >= ci.checked_in_at
       WHERE ci.restaurant_id = $1
       GROUP BY ci.id, u.first_name, u.last_name, m.serial_number, m.tier
  ...
  // GET /api/checkin/mine/latest — member: was I just checked in? (triggers the meal popup)
  app.get('/mine/latest', { preHandler: authenticate }, async (req, reply) => {
    const ci = await db.findOne(
      `SELECT ci.id, ci.checked_in_at, ci.restaurant_id, r.name AS restaurant_name
       FROM check_ins ci
    ...
    if (!ci) return { checkin: null }
    // Only "fresh" if within the last 3 hours
    const fresh = (Date.now() - new Date(ci.checked_in_at).getTime()) < 3 * 3600 * 1000
    return { checkin: fresh ? ci : null }
  })
...
// ═══ MEMBER MEALS — manual dish logging (Health tab) ═══
export async function mealRoutes(app) {
  // GET /api/meals/mine — the member's logged meals (with today's totals)
  app.get('/mine', { preHandler: authenticate }, async (req, reply) => {
    const meals = await db.findMany(
      `SELECT mm.*, r.name AS restaurant_name
       FROM member_meals mm
       JOIN restaurants r ON r.id = mm.restaurant_id
       WHERE mm.member_id = $1
       ORDER BY mm.eaten_at DESC
       LIMIT 100`,
      [req.user.id]
    )
    return meals
  ...
  // POST /api/meals — log a dish the member ate
  // body: { restaurantId, dishId?, dishName, calories, protein, carbs, fat, eatenAt? }
  app.post('/', { preHandler: authenticate }, async (req, reply) => {
    const { restaurantId, dishId, dishName, calories, protein, carbs, fat, eatenAt } = req.body || {}
    if (!restaurantId || !dishName) return reply.code(400).send({ error: 'restaurantId and dishName required' })
    // Must be a member of that restaurant
    const membership = await db.findOne(
      'SELECT id FROM memberships WHERE restaurant_id = $1 AND member_id = $2 AND status = $3',
      [restaurantId, req.user.id, 'active']
    )
    if (!membership) return reply.code(403).send({ error: 'Not a member of this restaurant' })

    const meal = await db.findOne(
      `INSERT INTO member_meals (member_id, restaurant_id, dish_id, dish_name, calories, protein, carbs, fat, eaten_at)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8, COALESCE($9, NOW())) RETURNING *`,
      [req.user.id, restaurantId, dishId || null, dishName,
       parseInt(calories)||0, parseInt(protein)||0, parseInt(carbs)||0, parseInt(fat)||0, eatenAt || null]
    )
  ...
  app.delete('/:id', { preHandler: authenticate }, async (req, reply) => {
    await db.run('DELETE FROM member_meals WHERE id = $1 AND member_id = $2', [req.params.id, req.user.id])
    return { success: true }
  })
}
